...
institution_auth_routes = APIRouter(
    prefix='/institution', tags=['Institution']
)


@institution_auth_routes.post(
    '/auth/create', response_model=SucessResponse, status_code=201
)
...
@institution_auth_routes.post(
    '/auth/login', response_model=SucessResponse, status_code=200
)
async def login_institution_auth(
    ...
    try:
        institution_login = await service.login_institution(
