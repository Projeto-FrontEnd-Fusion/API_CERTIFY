...
class CertificateService:
    ...
    async def create_participant_certificate(
        ...
        if not isExistingUser:
            raise Exception(
                'Usuário não existe, certificado não pode ser criado'
            )
    ...
    async def get_many_certificates(
        ...
        certificates = await self.certificate_repository.get_many_certificates(
            user_id
        )
        return certificates
    ...
    async def get_certificate_by_id(
        ...
        response = await self.certificate_repository.get_certificate(
            certificate_id
        )
